...
class SoftwareIDsGetter:
    ...
    def __init__(self, logger: logging.Logger = None) -> None:
        if logger is None:
            self.logger = loggit.get_default_logger(
                console=False,
                set_level_to="INFO",
                log_name="logs/get_software_ids_logs.txt",
            )
        else:
            self.logger = logger
    ...
    def get_software_ids(
        ...
        if total_records < per_pg:
            ...
            self.logger.debug(
                "total_records is less than per_pg; adjusting per_pg to total_records."
            )
        ...
        s = requests.Session()
        retries = Retry(
            total=5, backoff_factor=1, status_forcelist=[202, 502, 503, 504]
        )
        ...
        for calls in range(requests_needed):
            ...
            self.logger.debug(
                f"API response on request {page_iterator}: {api_response}"
            )
            self.logger.debug(
                f"API rate limit remaining: {api_response.headers['x-ratelimit-remaining']}"
            )
        ...
        if len(identifiers) > total_records:
            self.logger.debug(
                f"Slicing down list of IDs from {len(identifiers)} to {total_records}."
            )
        ...
        software_ids_df.to_csv(
            write_out_extra_info, index=True, header=False, mode="w", sep=","
        )
        self.logger.info(
            f"{len(identifiers)} Zenodo software IDs saved out as: {write_out_extra_info} at {write_out_location}"
        )
...
if __name__ == "__main__":
    ...
    logger = loggit.get_default_logger(
        console=True, set_level_to="DEBUG", log_name="logs/get_software_ids_logs.txt"
    )
    ...
    software_ids = []
    try:
        software_ids = getter.get_software_ids(
            config_path="zenodococode/zenodoconfig.cfg",
            per_pg=20,
            total_records=10000,
            out_filename="zenodo_ids",
        ...
        logger.error(
            f"There's been an exception while trying to run get_software_ids(): {e}"
        )
