    ...
    def __init__(self, logger: logging.Logger = None) -> None:
        if logger is None:
            self.logger = loggit.get_default_logger(
                console=False, set_level_to="INFO", log_name="logs/get_gh_urls_logs.txt"
            )
        else:
            self.logger = logger
    ...
    def get_gh_urls(
        ...
        for i in chunker.chunker(zenodo_ids["zenodo_id"], batch_size):
            ...
            for record_id in writeable_chunk_ids:
                id_count += 1
                self.logger.debug(
                    f"Running loop for record ID {record_id}, in loop {loop_num}, ID count {id_count} of {total_records}"
                )
                ...
                try:
                    ...
                    if "metadata" in api_response.json():
                        ...
                        if "related_identifiers" in record_metadata:
                            record_metadata_identifiers = api_response.json()[
                                "metadata"
                            ]["related_identifiers"]

                            if (
                                "github.com"
                                in record_metadata_identifiers[0]["identifier"]
                ...
                # handle errors
                    ...
                    if verbose:
                        self.logger.error(
                            f"API response fail exception for {record_id}: {e}"
                        )
                    print(type(e))
                    print(e)
            ...
            gh_urls_df = pd.concat(
                [gh_urls_df, writeable_chunk_df],
            )
        ...
        if record_count >= (total_records / 2):
            ...
            self.logger.debug(
                "Less than half of records have GitHub urls... Something could be wrong?"
            )
...
if __name__ == "__main__":
    ...
    logger = loggit.get_default_logger(
        console=True, set_level_to="DEBUG", log_name="logs/get_gh_urls_logs.txt"
    )
    ...
    try:
        ...
        gh_urls_df = url_getter.get_gh_urls(
            config_path="zenodococode/zenodoconfig.cfg",
            in_filename="zenodo_ids",
            read_in_location="data/",
            out_filename="gh_urls",
        ...
        if len(gh_urls_df) != 0:
            ...
            logger.warning(
                "GitHub URL grab did not work, length of records returned is zero."
            )
    except Exception as e:
        logger.error(
            f"There's been an exception while trying to run get_gh_urls(): {e}"
        )
    ...
    try:
        ...
        try:
            assert (
                len(gh_urls_df.index) == len(total_urls.index)
            ), f"WARNING! Lengths of returned df ({len(gh_urls_df)}) vs df read in from file ({len(total_urls)}) DO NOT MATCH. Did you append too many records to the gh_urls file??"
