...
def test_get_all_branches_commits_N_branches():
    #     # Arrange:
    expectedjson = setup_deduplicated_data(deduplicatedjson)
    ...
    # Assert:
    assert (
        len(expectedjson) == len(output_get_all_branches_commits)
    ), f"The length of reading in deduplicated json {deduplicatedjson} and length of output generated by running get_all_branches_commits() on test repo 'FlicAnderson/peramagroon' are different. Check this!"
