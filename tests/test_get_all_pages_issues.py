...
def test_1pg_only():
    ...
    repo_links = requests.get(
        url=urlparam, params={"per_page": per_pg, "state": "all"}
    ).links
    repo_links_pages = [key for key in repo_links]
    if "last" in repo_links_pages:
        raise KeyError(
            f"test variable repo_name_a {repo_name_a} now contains more than one page, choose new test repo name with fewer issues as a test."
        )
    else:
        # continue test.
        content = issuepages.get_all_pages_issues(repo_name=repo_name_a)
        assert (
            len(content.index) <= per_pg
        ), "More issues retrieved than per_pg value for this one-page repo; check for duplication or other problems."
...
def test_open_issues_count():
    ...
    assert (
        int(get_all_pgs_open_iss) == int(open_iss)
    ), "Number of open issues in all_issues df does not match repo_connection.open_issues_count"
