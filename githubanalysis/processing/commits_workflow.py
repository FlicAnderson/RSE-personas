...
class RunCommits:
    ...
    def getcommitschangesvcats(
        ...
        """
        Loops through commits in processed commits df (generated by `reformat_commits_object()`),
        and generates temporary dataframe per commit, including number of changes per commit, number
        of files per commit, and filenames of those changed files per commit, from tuples returned
