...
class IssueGetter: 
    ...
    def get_all_pages_issues(self, repo_name, config_path='githubanalysis/config.cfg', out_filename='all-issues', write_out_location='data/', issue_state='all'):
        ...
        if len(sys.argv) == 2:
            ...
            self.logger.info(f"Using commandline argument {repo_name} as repo name to retrieve GH issues for. Entered as: {sys.argv[1]}")
        else: 
        ...
        if api_response.status_code != 404:     
            ...
            try: 
                if 'last' in issue_links:
                    ...
                    for i in pg_range: 
                        pg_count += 1
                        self.logger.info(f">> Running issue grab for repo {repo_name}, in page {pg_count} of {pages_issues}.")
                    ...
                    if not json_pg: # check emptiness of result.
                        ...
                        self.logger.error("Result of API request is an empty json. Error - cannot currently handle this result nicely.")
...
if __name__ == "__main__":
    ...
    try: 
        ...
        issues_file_extra_info = f"{issues_file}_{sanitised_repo_name}_{current_date_info}.csv"
        total_issues = pd.read_csv(issues_file_extra_info, header=0)
    except Exception as e:
        logger.error(f"There's been an exception while trying to read back in data generated by get_all_pages_issues() from {issues_file_extra_info}: {e}")

    try:
        assert  len(issues_df.index) == len(total_issues.index), f"WARNING! Lengths of returned df ({len(issues_df)}) vs df read in from file ({len(total_issues)}) DO NOT MATCH. Did you append too many records to the gh_urls file??"
