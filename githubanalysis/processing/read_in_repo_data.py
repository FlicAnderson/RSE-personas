...
def read_in_repo_data(
    ...
    if repo_name is None:
        ...
        repo_name = read_in_filename.split(
            "__"
        )[
            1
        ]  # e.g. read_in_filename = 'all_issues__riboviz_riboviz' -> ['all_issues', 'riboviz_riboviz']
        repo_name = repo_name.replace(
            "_", "/"
        )  # get back to user/repo format with slashes instead of underscores.
    ...
    try:
        ...
        if verbose:
            if read_in_df.empty:  # if read_in hasn't worked and dataframe is empty
                ...
                print(
                    f"Dataframe has been created from file {read_in} for repo {repo_name} and has dimensions: {read_in_df.shape}."
                )
        ...
        raise Exception(
            f"Reading in file {read_in} from repo {repo_name} did not work."
        )
