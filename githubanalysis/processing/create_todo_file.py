...
class TODOMaker:
    ...
    def __init__(self, logger: logging.Logger = None) -> None:
        if logger is None:
            self.logger = loggit.get_default_logger(console=False, set_level_to='INFO', log_name='logs/create_todo_file_logs.txt')
        else:
            self.logger = logger
                
    def create_todo_file(self, in_filename='gh_urls', read_in_location='data/', out_filename='todo', write_out_location='data/'):
        """
        ...
        if len(sys.argv) == 2:
            gh_urls_file_extra_info = sys.argv[1]  # use second argv (user-provided by commandline)
        ...
        try: 
            gh_urls_df = pd.read_csv(gh_urls_file_extra_info, header=0, usecols=['ZenodoID', 'GitHubURL'], dtype={'ZenodoID':'Int64', 'GitHubURL':'str'})
            self.logger.debug(f"Read in {len(gh_urls_df.index)} records to convert to TODO file.")
        except Exception as e: 
            self.logger.error(f"There's been an exception while trying to read back in data generated by get_gh_urls() from {gh_urls_file_extra_info}: {e}")
        
        # pull out usable repo name from url string
        try: 
            gh_urls_df['repo_name'] = gh_urls_df[['GitHubURL']].apply(lambda x: [getreponame.repo_name_clean(x) for x in x])
        except Exception as e: 
            self.logger.error(f"Issue while trying to apply repo_name_clean() to repo urls: {e}")
        ...
        gh_urls_df['check_uses_tickets'] = f"NOT Started @ {datetime.now().strftime('%Y-%m-%d, %H:%M')}"
        gh_urls_df['check_number_tickets'] = f"NOT Started @ {datetime.now().strftime('%Y-%m-%d, %H:%M')}"
        gh_urls_df['check_number_contributors'] = f"NOT Started @ {datetime.now().strftime('%Y-%m-%d, %H:%M')}"
        gh_urls_df['check_uses_PRs'] = f"NOT Started @ {datetime.now().strftime('%Y-%m-%d, %H:%M')}"
        gh_urls_df['check_number_PRs'] = f"NOT Started @ {datetime.now().strftime('%Y-%m-%d, %H:%M')}"
        gh_urls_df['check_repo_age'] = f"NOT Started @ {datetime.now().strftime('%Y-%m-%d, %H:%M')}"
        gh_urls_df['check_repo_active'] = f"NOT Started @ {datetime.now().strftime('%Y-%m-%d, %H:%M')}"
        gh_urls_df['check_number_commits'] = f"NOT Started @ {datetime.now().strftime('%Y-%m-%d, %H:%M')}"
        gh_urls_df['check_repo_license'] = f"NOT Started @ {datetime.now().strftime('%Y-%m-%d, %H:%M')}"
        gh_urls_df['check_repo_visibility'] = f"NOT Started @ {datetime.now().strftime('%Y-%m-%d, %H:%M')}"
        gh_urls_df['check_repo_language'] = f"NOT Started @ {datetime.now().strftime('%Y-%m-%d, %H:%M')}"

        # write out todo file (use added date filename)
        try: 
            gh_urls_df.to_csv(write_out_extra_info, mode='w', index=False, header=True)
            self.logger.info(f"Written out {len(gh_urls_df.index)} github url records to TODO file {write_out_extra_info} with {gh_urls_df.shape[1]} columns.")
        except Exception as e:
            self.logger.error(f"There's been an exception while trying to write out todo list generated from {gh_urls_file_extra_info} into {write_out_extra_info}: {e}")
...
if __name__ == "__main__":
    ...
    """
    logger = loggit.get_default_logger(console=True, set_level_to='DEBUG', log_name='logs/create_todo_file_logs.txt')

    todo_maker = TODOMaker(logger)

    gh_urls_df = pd.DataFrame()
    try: 
        todo_maker.create_todo_file(in_filename='gh_urls', read_in_location='data/', out_filename='todo', write_out_location='data/')
    except Exception as e:
