...
class DevsGetter:
    ...
    def __init__(self, logger: logging.Logger = None) -> None:
        if logger is None:
            self.logger = loggit.get_default_logger(
                console=False,
                set_level_to="INFO",
                log_name="logs/get_repo_contributors_logs.txt",
            )
        else:
            self.logger = logger
    ...
    def get_repo_contributors(
        ...
        if len(sys.argv) == 2:
            ...
            self.logger.info(
                f"Using commandline argument {repo_name} as repo name to retrieve GH contributors for. Entered as: {sys.argv[1]}"
            )
        else:
            try:
                repo_name = repo_name
        ...
        if api_response.status_code != 404:
            ...
            try:  # get contributors data from page
                if "last" in contributors_links:
                    contributors_links_last = contributors_links["last"]["url"].split(
                        "&page="
                    )[1]
                    ...
                    for i in pg_range:
                        pg_count += 1
                        self.logger.info(
                            f">> Running contributors grab for repo {repo_name}, in page {pg_count} of {pages_contributors}."
                        )
                        ...
                        if len(store_pg.index) > 0:
                            ...
                            contributors_df = pd.concat(
                                [contributors_df, store_pg],
                            )  # append this page (df) to main contributors df
                    ...
                    try:
                        pg_count += 1
                        self.logger.debug(
                            f"getting json via request url {contributors_url}."
                        )
                        json_pg = api_response.json()
                        ...
                        # store_pg['login'] = store_pg['login'].replace(to_replace=r'^\s*$', value='Anonymous', regex=True)  # if login is empty, this is likely due to Anonymous user. Fill with 'Anonymous'
                        self.logger.debug(
                            f"store_pg df contains columns: {store_pg.columns}; 'login' in store_pg df?: {'login' in store_pg.columns}."
                        )
                        contributors_df = store_pg
                        ...
                        self.logger.error(
                            f"Single page of contributors getting failed for repo {repo_name}: {e_singlepg}"
                        )

                self.logger.info(
                    f"Total number of contributors grabbed is {len(contributors_df.index)} in {pg_count} page(s)."
                )
                self.logger.info(
                    f"There are {contributors_df['login'].isna().sum()} anonymous contributors for repo {repo_name}."
                )

            except Exception as e_contributors:
                self.logger.error(
                    f"Something failed in getting contributors for repo {repo_name}: {e_contributors}"
                )
            ...
            if "login" in contributors_df.columns:
                if contributors_df["login"].isna().sum() > 499:
                    self.logger.warning(
                        f"There are 500+ ({contributors_df['login'].isna().sum()})contributors in repo {repo_name}; GH only lists 500 users fully, the rest will be shown as anonymous (see https://docs.github.com/en/rest/repos/repos?apiVersion=2022-11-28#list-repository-contributors)."
                    )
            else:
                self.logger.warning(
                    f"There is no 'login' in contributors_df; {contributors_df.columns}; adding 'login' column and filling with NA values."
                )
                contributors_df["login"] = (
                    pd.NA
                )  # this will NOT differentiate between separate Anonymous devs
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(contributors_df) != 0:
            ...
            logger.warning(
                "Getting contributors did not work, length of returned records is zero."
            )
    except Exception as e:
        logger.error(
            f"Exception while running get_repo_contributors() on repo {repo_name}: {e}"
        )
    ...
    try:
        ...
        contributors_file_extra_info = (
            f"{contributors_file}_{sanitised_repo_name}_{current_date_info}.csv"
        )
        total_contributors = pd.read_csv(contributors_file_extra_info, header=0)
    except Exception as e:
        logger.error(
            f"There's been an exception while trying to read back in data generated by get_all_pages_contributors() from {contributors_file_extra_info}: {e}"
        )

    try:
        assert (
            len(contributors_df.index) == len(total_contributors.index)
        ), f"WARNING! Lengths of returned df ({len(contributors_df)}) vs df read in from file ({len(total_contributors)}) DO NOT MATCH. Did you append too many records to the gh_urls file??"
