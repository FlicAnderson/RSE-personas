    ...
    def __init__(self, logger: logging.Logger = None) -> None:
        if logger is None:
            self.logger = loggit.get_default_logger(
                console=False,
                set_level_to="INFO",
                log_name="logs/gather_gh_stats_logs.txt",
            )
        else:
            self.logger = logger
    ...
    def gather_gh_stats(
        ...
        if len(sys.argv) == 2:
            ...
            if not isinstance(todo_file, str):
                raise TypeError(
                    "Ensure argument is a file location and name in string format (e.g. 'data/todo.csv')"
                )

            self.logger.info(
                f"Using commandline argument {todo_file} as input file of GH Repo names to retrieve GH stats for. Entered as: {sys.argv[1]}"
            )
        else:
        ...
        gh_repos_df = pd.read_csv(
            todo_file,
            header=0,
            usecols=["ZenodoID", "repo_name"],
            dtype={"ZenodoID": "Int64", "repo_name": "str"},
        )
        ...
        for i in chunker.chunker(gh_repos_df["repo_name"], batch_size):
            ...
            writeable_chunk_repos = (
                i.to_list()
            )  # pull IDs into a list of size 10 in this iteration of the loop

            for repo in writeable_chunk_repos:
                id_count += 1
                self.logger.debug(
                    f">> Running loop for record ID {repo}, in loop {loop_num}, at repo {id_count} of {total_records}"
                )
                # print(f">> Getting data for repo {repo}, in loop {loop_num}, which is repo {id_count} of {total_records}. <<<")

                try:
                    repo_stats = RepoStatsSummariser.summarise_repo_stats(
                        self, repo_name=repo, config_path=config_path, per_pg=1
                    )
                    writeable_chunk.append(repo_stats)
                except Exception as e_repo:
                    self.logger.error(
                        f"Something failed in getting stats for repo {repo}, in loop {loop_num}, which is repo {id_count} of {total_records}: {e_repo}"
                    )
            ...
            gh_stats_df = pd.concat(
                [gh_stats_df, writeable_chunk_df],
            )
...
if __name__ == "__main__":
    ...
    logger = loggit.get_default_logger(
        console=True, set_level_to="DEBUG", log_name="logs/gather_gh_stats_logs.txt"
    )
    ...
    try:
        ...
        gh_repos_df = stat_getter.gather_gh_stats(
            config_path="githubanalysis/config.cfg",
            in_filename="todo",
            read_in_location="data/",
            out_filename="gh_stats",
        ...
        if len(gh_repos_df) != 0:
            ...
            logger.warning(
                "GitHub stats grab did not work, length of records returned is zero."
            )
    except Exception as e:
        logger.error(
            f"There's been an exception while trying to run gather_gh_stats(): {e}"
        )
    ...
    try:
        ...
        logger.error(
            f"There's been an exception while trying to read back in data generated by gather_gh_stats() from {gh_stats_file_extra_info}: {e}"
        )

        # if everything is good, this shouldn't trigger.
        try:
            assert (
                len(gh_repos_df.index) == len(total_stats.index)
            ), f"WARNING! Lengths of returned df ({len(gh_repos_df)}) vs df read in from file ({len(total_stats)}) DO NOT MATCH. Did you append too many records to the gh_urls file??"
