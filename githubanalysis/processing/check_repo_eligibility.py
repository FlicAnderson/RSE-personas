...
class GenerateStudySample(DatasetSetup):
    def _log_name(self) -> str:
        return "check_repo_eligibility"

    def __write_out_eligible_repos(
        self,
        ...
        df_writeout_path = Path(self.data_write_location, df_filestr)
    ...
    def check_repo_eligibility(
        ...
        try:
            ...
            self.__write_out_eligible_repos(study_sample_df=eligible_repos)
...
parser.add_argument(
    ...
    "--dataset-name",
    required=True,
    help="dataset run name, generates subfolder names etc",
    type=str,
)
...
if __name__ == "__main__":
    ...
    generate_study_sample = GenerateStudySample(
        in_notebook=False,
        logger=logger,
    ...
    try:
        if population_set is not None:
            ...
            if study_sample is None:
                raise RuntimeError(
                    f"No study sample returned; check logs/outputs; run time was: {datetime.datetime.now() - start_time}"
                )
        ...
        raise RuntimeError("failed to generate the study sample.") from e
