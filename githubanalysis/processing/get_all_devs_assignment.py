...
class DevsAssigner:
    ...
    def get_all_devs_assignment(self, repo_name, issues_filename='all-issues', contributors_filename='contributors', read_in_location='data/', out_filename='devs-assignments', write_out_location='data/'):
        ...
        self.logger.info(f"Dev Assignments have been written out to file {write_out_extra_info}")
...
if __name__ == "__main__":
    ...
    try:
        if len(sys.argv) == 4:
            ...
            repo_name = sys.argv[1]  # use second argv (user-provided by commandline)
            logger.debug(f"Using commandline argument {repo_name} as repo to get dev issue/PR assignments for. Entered as: {sys.argv[1]}. ")   

            issues_filename = issues_filename = sys.argv[2]  # use third argv (user-provided by commandline)
        ...
        logger.error(f"Exception whilst processing inputs while running get_all_Devs_assignments() as main(); {e_mainentry}; Please check inputs.")
    
    logger.debug(f"Commandline inputs: {repo_name}; {issues_filename}; {contributors_filename}.")
    ...
    try: 
        ...
        sanitised_repo_name = repo_name.replace("/", "-")
        dev_cats_file_extra_info = f"{dev_cats_file}_{sanitised_repo_name}_{current_date_info}.csv"
        total_dev_cats = pd.read_csv(dev_cats_file_extra_info, header=0)
    except Exception as e: 
        logger.error(f"There's been an exception while trying to read back in data generated by get_all_devs_assignment() from {dev_cats_file_extra_info}: {e}")

    # if everything is good, this shouldn't trigger.
    try: 
        assert len(devs_assignments_df.index) == len(total_dev_cats.index), f"WARNING! Lengths of returned df ({len(devs_assignments_df)}) vs df read in from file ({len(total_dev_cats)}) DO NOT MATCH."
