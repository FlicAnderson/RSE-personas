    ...
    def __init__(self, logger: logging.Logger = None) -> None:
        if logger is None:
            self.logger = loggit.get_default_logger(
                console=False,
                set_level_to="INFO",
                log_name="logs/get_all_devs_assignment_logs.txt",
            )
        else:
            self.logger = logger

    def get_all_devs_assignment(
        self,
        repo_name,
        issues_filename="all-issues",
        contributors_filename="contributors",
        read_in_location="data/",
        out_filename="devs-assignments",
        ...
        :type: string
        :param issues_filename: filename (prefix only, no '.csv' - e.g. 'all-issues_JeschkeLab-DeerLab_2024-02-27') of issues dataset generated by get_all_pages_issues(). Default: 'all-issues'
        :type: string
        :param contributors_filename: filename (prefix only, no '.csv' - e.g. 'contributors_JeschkeLab-DeerLab_2024-02-27') of contributor dataset generated by get_repo_contributors(). Default: 'contributors'
        :type: string
        ...
        if len(sys.argv) >= 2:
            ...
            self.logger.debug(
                f"Using commandline argument {repo_name} as repo name to get dev issue/PR assignments for. Entered as: {sys.argv[1]}. "
            )
        ...
        if len(sys.argv) >= 3:
            issues_filename = sys.argv[
                2
            ]  # use third argv (user-provided by commandline)
            self.logger.debug(
                f"Using commandline argument {issues_filename} as issues filename to get dev issue/PR assignments for. Entered as: {sys.argv[2]}. "
            )
            if not isinstance(issues_filename, str):
                raise TypeError(
                    "Ensure argument is issues dataset in string format (e.g. 'all-issues_JeschkeLab-DeerLab_2024-02-27')"
                )

            if len(sys.argv) >= 4:
                contributors_filename = sys.argv[
                    3
                ]  # use fourth argv as location of issues data if given
                self.logger.debug(
                    f"Using commandline argument {contributors_filename} as contributors filename to get dev issue/PR assignments for. Entered as: {sys.argv[3]}. "
                )
                if not isinstance(contributors_filename, str):
                    raise TypeError(
                        "Ensure argument is contributors dataset in string format (e.g. 'all-issues_JeschkeLab-DeerLab_2024-02-27')"
                    )
        else:
            try:
                repo_name = repo_name
                ...
                raise IndexError(
                    "Please enter a repo_name (NECCESSARY), issues_filename (optional) and contributors_filename (optional) all as 'quoted strings' separated by a space..."
                )

        print(f"{repo_name}; {issues_filename}; {contributors_filename}.")
        self.logger.info(f"{repo_name}; {issues_filename}; {contributors_filename}.")
        ...
        try:
            ...
            try:
                if os.path.isfile(issues_filename):
                    ...
                    self.logger.debug(
                        f"issues_filepath {issues_filepath} exists = {os.path.isfile(issues_filepath)}."
                    )
            except:
                FileExistsError(f"File {issues_filepath} does not seem to exist...?")
        except Exception as e_read_issues:
            self.logger.error(
                f"Error in reading in issues dataset ({issues_filepath}) for repo {repo_name}: {e_read_issues}"
            )
        ...
        try:
            ...
            self.logger.error(
                f"Error in reading in contributors dataset ({contributors_filepath}) for repo {repo_name}: {e_read_contributors}"
            )
        ...
        devs_assignments_df = pd.merge(
            all_devs_df, contributors_df, how="outer", on=["dev_name"]
        )

        # drop extra columns brought in from contributors dataset
        devs_assignments_df = devs_assignments_df[
            [
                "dev_name",
                "total",
                "PRs",
        ...
        devs_assignments_df["assignment"] = np.select(
            conditions, choices, default="error"
        )

        # remove non-named-dev entries from joined dataset
        devs_assignments_df = devs_assignments_df.drop(
            devs_assignments_df[devs_assignments_df["dev_name"] == "unassigned"].index
        )
        devs_assignments_df = devs_assignments_df.drop(
            devs_assignments_df[devs_assignments_df["dev_name"].isna()].index
        )

        # make assignment a category datatype for easier future work (optional in script probably)
        devs_assignments_df["assignment"] = devs_assignments_df["assignment"].astype(
            "category"
        )
        # set the other categories so whole set is there if not present from data
        devs_assignments_df["assignment"].cat.set_categories(
            ["both", "issues_only", "PRs_only", "neither"]
        )

        # change NaNs with no contributions (via join) to 0 to improve means and other stats
        devs_assignments_df["contributions"] = devs_assignments_df[
            "contributions"
        ].fillna(0)

        # write out dataset:
        devs_assignments_df.to_csv(
            write_out_extra_info, mode="w", index=True, header=True
        )
        self.logger.info(
            f"Dev Assignments have been written out to file {write_out_extra_info}"
        )
...
if __name__ == "__main__":
    ...
    try:
        if len(sys.argv) == 4:
            repo_name = sys.argv[1]  # use second argv (user-provided by commandline)
            logger.debug(
                f"Using commandline argument {repo_name} as repo to get dev issue/PR assignments for. Entered as: {sys.argv[1]}. "
            )

            issues_filename = issues_filename = sys.argv[
                2
            ]  # use third argv (user-provided by commandline)
            logger.debug(
                f"Using commandline argument {issues_filename} as issues filename to get dev issue/PR assignments for. Entered as: {sys.argv[2]}. "
    ...
    except Exception as e_mainentry:
        logger.error(
            f"Exception whilst processing inputs while running get_all_Devs_assignments() as main(); {e_mainentry}; Please check inputs."
        )

    logger.debug(
        f"Commandline inputs: {repo_name}; {issues_filename}; {contributors_filename}."
    )
    ...
    # run the function!
    try:
        devs_assignments_df = devs_assigner.get_all_devs_assignment(
            repo_name,
            issues_filename=issues_filename,
            contributors_filename=contributors_filename,
            read_in_location="data/",
        ...
        if len(devs_assignments_df) != 0:
            logger.info(
                f"Number of devs given assignment category for repo {repo_name} is {len(devs_assignments_df.index)}."
            )
            logger.info(
            ...
            logger.warning(
                "Getting all dev assignments did not work, length of returned records is zero."
            )
    except Exception as e:
        logger.error(
            f"Exception while running get_all_devs_assignment() on repo {repo_name}: {e}"
        )

    # attempt to read data back in as check:
    try:
        current_date_info = datetime.now().strftime(
            "%Y-%m-%d"
        )  # run this at start of script not in loop to avoid midnight/long-run issues
        dev_cats_file = "data/devs-assignments"
        sanitised_repo_name = repo_name.replace("/", "-")
        dev_cats_file_extra_info = (
            f"{dev_cats_file}_{sanitised_repo_name}_{current_date_info}.csv"
        )
        total_dev_cats = pd.read_csv(dev_cats_file_extra_info, header=0)
    except Exception as e:
        logger.error(
            f"There's been an exception while trying to read back in data generated by get_all_devs_assignment() from {dev_cats_file_extra_info}: {e}"
        )

    # if everything is good, this shouldn't trigger.
    try:
        assert (
            len(devs_assignments_df.index) == len(total_dev_cats.index)
        ), f"WARNING! Lengths of returned df ({len(devs_assignments_df)}) vs df read in from file ({len(total_dev_cats)}) DO NOT MATCH."
