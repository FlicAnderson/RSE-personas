"""Function to retrieve commits from default or specified branch for a given GitHub repository."""
...
class CommitsGetter:
    ...
    def get_all_pages_commits(
        ...
        :type: int
        :param branch: name of branch to get commits for within repo (Default: 'default' which fetches GH default branch, usually 'main')
        :type: str
        ...
        if len(sys.argv) == 2:
            ...
            self.logger.info(
                f"Using commandline argument {repo_name} as repo name to retrieve GH commits for. Entered as: {sys.argv[1]}"
            )
        else:
        ...
        try:
            ...
            if branch == "default" or "main":
                commits_url = f"{repos_api_url}{repo_name}/commits?per_page={per_pg}&page={page}"  # don't use branch in query, obtains GH default branch.
                branch = "main"
            else:
            ...
            try:
                if "last" in commit_links:
                    ...
                    for i in pg_range:
                        pg_count += 1
                        self.logger.info(
                            f">> Running commit grab for repo {repo_name}, on branch {branch}, in page {pg_count} of {pages_commits}."
                        )
                        page = i
                        ...
                        if len(store_pg.index) > 0:
                            try:
                                ...
                                store_pg["branch"] = branch
                                store_pg["commit_message"] = pd.DataFrame.from_dict(
                                    store_pg["commit"]
                                ).apply(lambda x: [x.get("message") for x in x])
                                store_pg["author_dev"] = store_pg["author"].str[
                                    "login"
                                ]  # via: @Mozway on SO: https://stackoverflow.com/a/71782066
                                store_pg["committer_dev"] = store_pg["committer"].str[
                                    "login"
                                ]  # via: @Mozway on SO: https://stackoverflow.com/a/71782066
                                store_pg["author_date"] = pd.DataFrame.from_dict(
                                    store_pg["commit"]
                                ).apply(
                                    lambda x: [x.get("author").get("date") for x in x]
                                )
                                store_pg["committer_date"] = pd.DataFrame.from_dict(
                                    store_pg["commit"]
                                ).apply(
                                    lambda x: [
                                        x.get("committer").get("date") for x in x
                                    ]
                                )
                                store_pg["same_date"] = np.where(
                                    (
                                        store_pg["author_date"]
                                        == store_pg["committer_date"]
                                    ),
                                    True,
                                    False,
                                )
                            ...
                            all_commits = pd.concat(
                                [all_commits, store_pg],
                            )  # append this page (df) to main commits df
                    ...
                    if len(store_pg.index) > 0:
                        try:
                            ...
                            store_pg["author_date"] = pd.DataFrame.from_dict(
                                store_pg["commit"]
                            ).apply(lambda x: [x.get("author").get("date") for x in x])
                            store_pg["committer_date"] = pd.DataFrame.from_dict(
                                store_pg["commit"]
                            ).apply(
                                lambda x: [x.get("committer").get("date") for x in x]
                            )
                            store_pg["same_date"] = np.where(
                                (store_pg["author_date"] == store_pg["committer_date"]),
                                True,
                                False,
                            )
                ...
                self.logger.info(
                    f"Total number of commits grabbed is {len(all_commits.index)} in {pg_count} page(s)."
                )
                self.logger.info(
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(commits_df) != 0:
            logger.info(
                f"Number of commits returned for repo {repo_name} is {len(commits_df.index)}."
            )
        else:
            logger.warning(
                "Getting commits did not work, length of returned records is zero."
            )
    except Exception as e:
        logger.error(
            f"Exception while running get_all_pages_commits() on repo {repo_name}: {e}. Traceback: {traceback.format_exc()}"
        )
    ...
    try:
        assert (
            len(commits_df.index) == len(total_commits.index)
        ), f"WARNING! Lengths of returned df ({len(commits_df)}) vs df read in from file ({len(total_commits)}) DO NOT MATCH. Did you append too many records to the gh_urls file??"
