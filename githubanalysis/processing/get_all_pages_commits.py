...
class CommitsGetter: 
    ...
    def get_all_pages_commits(self, repo_name, config_path='githubanalysis/config.cfg', per_pg=100, branch='default', out_filename='all-commits', write_out_location='data/'):
        """
        ...
        if api_response.status_code != (404 or 401):     
            ...
            try: 
                if 'last' in commit_links:
                    ...
                    for i in pg_range: 
                        pg_count += 1
                        self.logger.info(f">> Running commit grab for repo {repo_name}, on branch {branch}, in page {pg_count} of {pages_commits}.")        
                        page = i
