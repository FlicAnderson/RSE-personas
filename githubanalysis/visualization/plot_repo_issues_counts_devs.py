...
def plot_repo_issues_counts_devs(
    repo_data_df,
    repo_name,
    save_out=True,
    save_name="issues_counts_devs_plot",
    save_type="png",
    ...
    exploded_devs = repo_data_df.apply(pd.Series).explode(
        column="assigned_devs", ignore_index=True
    )
    ...
    non_assigned_text = (
        f"{all_closed - total_assigned} Unassigned tickets (N = {all_closed})"
    )

    if verbose:
        print(
            exploded_devs["assigned_devs"].value_counts(dropna=False)
        )  # print per-dev assignment counts

    # do actual plotting:
    sns.set_theme()
    sns.set_palette("colorblind")

    exploded_devs["assigned_devs"].value_counts().plot.bar(
        column="assigned_devs", color="red", label="assigned tickets"
    )
    ...
    plt.axhline(
        y=(total_assigned * 0.25),
        linestyle="--",
        color="black",
        label=f"25% ({total_assigned}) assigned issues",
    )  # plot line at 25% of all closed assigned tickets
    plt.plot(
        [], [], " ", label=non_assigned_text
