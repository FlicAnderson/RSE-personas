...
def plot_repo_issues_data(
    repo_data_df,
    repo_name,
    xaxis="ticket_number",
    add_events=False,
    save_out=True,
    save_name="issues_data_plot",
    save_type="png",
    ...
    if xaxis == "ticket_number":
        ...
        (
            ...
            .set(
                title=f"Time in days to close GitHub issues from {repo_name}",
                xlabel="GitHub Issue Ticket Number",
                ylabel="Time to Close Issue (days)",
            )
            .add_legend(title="Pull Requests?"),
            plt.axhline(
                y=np.mean(repo_data_df.close_time), linestyle="--", color="black"
            ),  # add mean line with average close time for this set of issues
        )
    ...
    if xaxis == "project_time":
        ...
        g.set_axis_labels(
            "Time Since Repo Creation (days)", "Time to Close Issue (days)"
        )
        g.add_legend(title="Pull Requests?")
        g.refline(
            y=np.mean(repo_data_df.close_time), linestyle="--", alpha=0.5
        )  # add mean line w/ average close time for repo

        if add_events is not False:
            for event in add_events:
                g.refline(
                    x=event, ymin=0, ymax=1, color="black", linestyle=":", alpha=0.75
                )
