...
class DevCatPlotter:
    ...
    def __init__(self, logger: logging.Logger = None) -> None:
        if logger is None:
            self.logger = loggit.get_default_logger(console=False, set_level_to='INFO', log_name='logs/plot_single_repo_dev_assignments_logs.txt')
        else:
            self.logger = logger
    
    def plot_single_repo_dev_assignments(self, repo_name, devs_assignments_file='devs-assignments', write_out_format='png',  read_in_location='data/', out_filename='plot-devs-assignments', write_out_location='images/'):
        ...
        try:
            ...
            self.logger.error(f"Exception while trying to read in dev_assignments file {dev_assignments_filepath} for repo {repo_name}: {e_readin}")
        ...
        sf1.suptitle(f'Repo {repo_name}; N devs = {len(dev_assignments.dev_name.index)}')
        (
            so.Plot(
            ...
            .add(so.Bars(width=.95, alpha=1), so.Count(), legend=False).scale(x=so.Nominal(order=cat_order), color=so.Nominal(palette_list, order=cat_order), legend=False)
        ...
        self.logger.info(f"Plot of developer assignment for repo {repo_name} written out to {write_out_format} file at {write_out_extra_info}.")
...
if __name__ == "__main__":
    ...
    logger = loggit.get_default_logger(console=True, set_level_to='DEBUG', log_name='logs/plot_single_repo_dev_assignments_logs.txt')
    ...
    try: 
        devs_plotter.plot_single_repo_dev_assignments(
            ...
            logger.error(f"Exception while running plot_single_repo_dev_assignments() on repo {repo_name} for file {devs_assignments_file}: {e}")